...
## Summary
- 1110 nodes · 2175 edges · 85 communities (69 shown, 16 thin omitted)
...
Cohesion: 0.12
Nodes (17): 3.1 Ingestion Pipeline (`ingestion_pipeline.py`), 3.2 Document Loaders (`document_loaders/`), 3.3 Structure-Aware Chunker (`structure_aware_chunker.py`), 3.4 Embedding Service (`embedding_service_optimized.py`), 3.6 Retrieval Service (`retrieval_service.py`), 3.8 Context Builder Service (`context_builder_service.py`), 3.9 Chat Service (`chat_service.py`), 3. KEY SERVICES & ARCHITECTURE (+9 more)
...
Cohesion: 0.5
Nodes (4): 3.7 Query Processing Service (`query_processing_service.py`), code:python (# Auto-extract from query text), code:python (expand_query(processed_query) -> List[str]), code:python (@dataclass)
